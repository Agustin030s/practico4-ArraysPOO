...
const proyectos = [
  "Ecommerce",
  "Microservicios con docker para sistema bancario",
  "Aplicacion de ventas en .NET",
];
...
const cambiarEstadoDeProyecto = (nombreProyecto, estado) => {
  const indexProyecto = proyectos.findIndex((proyecto) =>
    proyecto.includes(nombreProyecto)
  );
...
const asignarResponsableAlProyecto = (nombreProyecto, responsable) => {
  ...
  if (indexProyecto !== -1) {
    if (responsables[indexProyecto] === "Sin Asignar") {
      ...
      return alert(
        `Al proyecto ${proyecto} se le asigno el responsable ${responsable}`
      );
    } else {
      if (
        confirm(
          `El proyecto ${proyecto} ya tiene un responsable, desea cambiarselo?`
        )
      ) {
        responsables[indexProyecto] = responsable;
        return alert(
      ...
      return alert(
        `El proyecto ${proyecto} tiene como responsable a ${responsables[indexProyecto]}`
      );
    }
  } else {
    return alert(`No se puedo encontrar el proyecto ingresado`);
...
const resumenProyectosEnCurso = () => {
  const proyectosEnCurso = proyectos.filter((proyecto, index) =>
    proyectoEnCurso(estadosProyecto[index])
  );
  mostrarArrays(proyectosEnCurso, "Proyectos en curso");
};

const nombreProyecto = prompt(
  "Ingrese el proyecto al que desea cambiar el estado"
);
// const estadoNuevo = prompt('Ingrese el nuevo estado del proyecto');
const responsable = prompt(
  "Ingrese el nombre del responsable que asignara al proyecto"
);
